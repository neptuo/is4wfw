    ...
    private function formValidateAgainstTemplate($formId, $templateContent) {
		...
		private function showForm($formId, $templateContent, $rowId) {
			...
			foreach($this->ResourcesToAdd as $res) {
				switch($res[0]) {
					case 'style' : $return .= '<link rel="stylesheet" type="text/css" href="'.$res[1].'" />'; break;
					case 'script': $return .= '<script type="text/javascript" src="'.$res[1].'"></script>'; break;
				}
			}
			$return .= $fcontent;
			$return .= '</form>';
