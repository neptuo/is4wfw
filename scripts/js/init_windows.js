...
var AjaxLinksGlobal = new Links(document, 5, 'xml', true);
AjaxLinksGlobal.setUpdateLocation(false);
AjaxLinksGlobal.beforeRequest = function() {
	showLoading();
};
	
AjaxLinksGlobal.onSuccess = function(xmlHttp) {
...
function initRefreshButton(event) {
	...
	var refreshCover = document.createElement('div');
	refreshCover.className = 'refresh-cover';
	
	var checkbox = document.createElement('input');
	checkbox.id = 'ajaxCheckboxrefreshCover';
	...
	addEvent(checkbox, 'change', function() {
		if(checkbox.checked == true) {
			...
			AjaxCountDown.onReachZero = function() {
			
			};
		}
	}, false);
	refreshCover.appendChild(checkbox);
	
	var label = document.createElement('label');
	label.setAttribute('for', 'ajaxCheckboxrefresh');
	label.innerHTML = 'Auto redirect.';
	refreshCover.appendChild(document.createTextNode(' '));
	refreshCover.appendChild(label);
	
	refreshCover.appendChild(document.createElement('br'));
	
	var checkbox2 = document.createElement('input');
	checkbox2.id = 'ajaxCheckboxrefreshCover2';
	checkbox2.type = 'checkbox';
	checkbox2.checked = '';
	addEvent(checkbox2, 'change', function() {
		if(checkbox2.checked == true) {
			AjaxCountDown.onReachFive = function() {
				button.click();
			};
		} else {
			AjaxCountDown.onReachFive = function() {
	...
	refreshCover.appendChild(checkbox2);
	
	var label2 = document.createElement('label');
	label2.setAttribute('for', 'ajaxCheckboxrefreshCover2');
	label2.innerHTML = 'Auto refresh.';
	refreshCover.appendChild(document.createTextNode(' '));
	refreshCover.appendChild(label2);
	
	refresh.appendChild(refreshCover);
}
	...
	for(var i = 0; i < forms.length; i ++) {
		if(forms[i].name == 'select-project') {
		  ...
		  pform.setResponseTemplate('xml');
			...
			pform.beforeSubmit = formBeforeSubmit;
			pform.onSuccess = function(xmlhttp) {
				processAjaxResult(xmlhttp);
				$.blockUI({ message: "<h3>Some windows needs to be refreshed after project has been switched!</h3>" }); 
 				setTimeout($.unblockUI, 2500); };
			pform.setUseHashUrlIfFormActionIsPlain(true);
